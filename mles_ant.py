...
TASK_DESCRIPTION = """
Design a direct programmatic controller for Gymnasium Ant-v4.

The controller receives the raw Ant-v4 observation vector:
- obs shape: (27,)
- action shape: (8,), continuous torques clipped to [-1, 1]
- objective: maximize forward locomotion reward while keeping the ant healthy
...
def policy(obs: np.ndarray, t: int, memory: dict) -> np.ndarray:
    """Direct Ant-v4 controller.

    Args:
        obs: raw observation, shape (27,).
        t: current step in this episode.
        memory: mutable per-episode dict for simple controller state.

    Returns:
        action: np.ndarray, shape (8,), clipped to [-1, 1].
    """
    phase = 0.08 * t
    gait = np.array([
        np.sin(phase),
...
def _as_action(value: Any) -> np.ndarray:
    ...
    if action.shape != (8,):
        raise ValueError(f"policy must return shape (8,), got {action.shape}")
...
def _extract_position(env) -> tuple[float, float, float]:
    ...
    z = float(qpos[2]) if qpos.shape[0] > 2 else 0.0
    return x, y, z
...
class AntDirectPolicyEvaluation:
    def __init__(
        self,
        save_dir: Path,
        seeds: list[int],
        ...
        class _Evaluation(Evaluation):
            def __init__(self, outer: AntDirectPolicyEvaluation):
                super().__init__(
    ...
    def _exit_budget_reached(self):
        print(f"MLES Ant reached candidate budget: {self.max_evaluations}. Results: {self.save_dir}")
        sys.stdout.flush()
        sys.stderr.flush()
        os._exit(0)
    ...
    def evaluate_program(self, program_str: str, callable_func: callable):
        ...
        image_b64 = _trajectory_image(
            trajectories,
            title=f"Ant-v4 {candidate_id} reward={score:.1f}",
...
def main():
    parser = argparse.ArgumentParser(description="LLM4AD MLES direct-policy baseline for Ant-v4")
    ...
    save_dir = Path(args.save_dir or PROJECT_DIR / "runs" / f"ant_mles_{timestamp}")
    ...
    evaluator = AntDirectPolicyEvaluation(
        save_dir=save_dir,
        seeds=args.seeds,
    ...
    print(f"MLES Ant direct-policy run finished. Results: {save_dir}")
