...
IDEA_API_KEY = os.environ.get("DEEPSEEK_API_KEY", "").strip()
...
def call_idea_llm(system_prompt: str, user_prompt: str, step: str = "") -> str:
    if not IDEA_API_KEY:
        raise ValueError("DEEPSEEK_API_KEY is required. Do not put API keys in source code.")
...
{network_code_notes}"""
    ...
    response = call_idea_llm(system, prompt, step=f"G{gen}_I{idx}-RandomInit")
...
def _train_one_individual(ind: Individual, worker_id: int, round_idx: int) -> dict:
    ...
    import tempfile
    temp_dir = tempfile.mkdtemp(prefix=f"random_train_{ind.label}_r{round_idx}_")
...
def run_final_selection(elite_population: list, run_dir: str, logger):
    ...
    for ind in elite_population:
        ...
        logger.log(
            ...
            f"(std={std_reward:.2f}) [search_fit={ind.fitness:.2f}]"
        )

    aggregated.sort(key=lambda e: e["search_fitness"], reverse=True)
...
def random_search_loop(run_prefix: str):
    global LOG, RUN_DIR
    ...
    for gen in range(1, NUM_GENERATIONS + 1):
        LOG.log(f"\n{'='*80}")
        LOG.log(f"[第{gen}代] 随机生成新种群")
        LOG.log(f"{'='*80}")


        LOG.log(f"\n  随机生成 {POPULATION_SIZE} 个新个体（并行）")
        new_population = []

        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = [executor.submit(llm_init_individual, i, gen) for i in range(POPULATION_SIZE)]
            for future in as_completed(futures):
                ind = future.result()
                new_population.append(ind)
                LOG.log(f"    ✓ 完成 {ind.label}")

        new_population.sort(key=lambda x: x.idx)


        LOG.log(f"\n  评估第{gen}代种群...")
        new_population = run_population(new_population)

        best_current = max(new_population, key=lambda x: x.fitness)
        LOG.log(f"\n  第{gen}代完成: 最佳={best_current.label}(fitness={best_current.fitness:.2f}), "
                f"平均fitness={sum(ind.fitness for ind in new_population)/len(new_population):.2f}")


        if best_current.fitness > all_time_best.fitness:
            all_time_best = best_current
            all_time_best_gen = gen
            LOG.log(f"  ★ 发现新的历史最佳个体: {all_time_best.label} (fitness={all_time_best.fitness:.2f})")
        ...
        gen_summary = {
            ...
            "best_fitness": max(ind.fitness for ind in new_population),
            "mean_fitness": sum(ind.fitness for ind in new_population) / len(new_population),
            "all_time_best": {
                "label": all_time_best.label,
                "fitness": all_time_best.fitness,
                "generation": all_time_best_gen
            }
        }
    ...
    LOG.log(f"历史最佳个体: {all_time_best.label} (来自第{all_time_best_gen}代)")
    LOG.log(f"历史最佳fitness: {all_time_best.fitness:.2f}")
    ...
    final_best_summary = {
        "best_individual": {
            ...
            "generation": all_time_best_gen,
            "fitness": all_time_best.fitness,
            "network_design": all_time_best.network_design,
        },
    }
    with open(os.path.join(RUN_DIR, "final_best_individual.json"), "w", encoding="utf-8") as f:
        json.dump(final_best_summary, f, indent=2, ensure_ascii=False)
    ...
    if not RUN_FINAL_SELECTION:
        LOG.log(f"\n[跳过] RUN_FINAL_SELECTION=False，不对精英个体做最终筛选。")
        LOG.close()
        return

    elite_individuals = [all_time_best]
    LOG.log(f"\n{'='*80}")
    LOG.log(f"最终精英详细评估：")
    LOG.log(f"  随机搜索对照实验仅评估历史最佳个体 {all_time_best.label} (第{all_time_best_gen}代)")
    if all_time_best.label == gen0_best_label:
        LOG.log(f"  注意: 历史最佳个体就是第0代最佳个体 {gen0_best_label}")
    else:
        LOG.log(f"  第一代最佳个体 {gen0_best_label} 仅保存记录，不做5M详细评估")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="随机搜索实验 - EVOM消融实验")
    parser.add_argument("--task", type=str, required=True,
                        choices=list(TASK_CONFIGS.keys()),
                        help=f"选择任务: {', '.join(TASK_CONFIGS.keys())}")
    ...
    parser.add_argument("--gpu-ids", type=int, nargs="+", default=None,
                        help="最终精英评估使用的GPU编号列表 (默认: 4 5 6)")
